...
function launchParticlesJS(tagId, params) {
  let canvasEl = document.querySelector("#" + tagId + " > canvas");
  ...
  pJS.fn.canvasPaint = function () {
    pJS.canvas.ctx.fillStyle = pJS.canvas.bgColor;
  ...
  pJS.fn.particlesCreate = function () {
    ...
    pJS.fn.vendors.distanceParticles = function (p1, p2) {
        ...
        if (dist <= pJS.particles.line_linked.distance) {
            /* draw the line */
            var colorLine = pJS.particles.line_linked.color_rgb_line
            pJS.canvas.ctx.beginPath()
            pJS.canvas.ctx.strokeStyle = 'rgba(' + colorLine.r + ',' + colorLine.g + ',' + colorLine.b + ',' + (pJS.particles.line_linked.opacity - dist / pJS.particles.line_linked.distance) + ')'
            ...
            if (pJS.particles.line_linked.condensed_mode.enable) {
                let idx = p1.x - p2.x
                let idy = p1.y - p2.y
                let ax = idx / (pJS.particles.line_linked.condensed_mode.rotateX * 10 * 10 * 10)
                let ay = idy / (pJS.particles.line_linked.condensed_mode.rotateY * 10 * 10 * 10)
                p2.vx += ax
                p2.vy += ay
            }
        }
    ...
    pJS.fn.vendors.interactivity.grabParticles = function (p1, p2) {
        ...
        var dx_mouse = p1.x - pJS.interactivity.mouse.posX,
            dy_mouse = p1.y - pJS.interactivity.mouse.posY,
        ...
        if (dist <= pJS.particles.line_linked.distance && dist_mouse <= pJS.interactivity.mouse.distance && pJS.interactivity.status == 'mousemove') {
            ...
            var colorLine = pJS.particles.line_linked.color_rgb_line
            pJS.canvas.ctx.beginPath()
            pJS.canvas.ctx.strokeStyle = 'rgba(' + colorLine.r + ',' + colorLine.g + ',' + colorLine.b + ',' + (pJS.interactivity.line_linked.opacity - dist_mouse / pJS.interactivity.mouse.distance) + ')'
            pJS.canvas.ctx.moveTo(p1.x, p1.y)
...
/* --- LAUNCH --- */
export default function (tagId, params) {
